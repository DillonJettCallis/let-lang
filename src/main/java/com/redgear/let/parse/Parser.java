...
public class Parser {
    ...
    @SuppressWarnings("unchecked")
    private <T extends Token> T nextValue(Class<T> type, String expected) {
        Token next = next();

        if(type.isInstance(next) && expected.equals(next.getValue())) {
            return (T) next;
        } else {
            throw syntaxError(next, expected);
        }
    ...
    private Expression readExpression() {
        ...
        if(begin instanceof LiteralToken) {
            ...
            } else if(unaryOps.contains(op) || "-".equals(op)) {
    ...
    private Expression readExpression(Expression left) {
        ...
        if(begin instanceof OperatorToken) {
            ...
            if("});,".contains(op)) {
                ...
                return readExpression(readCall(left));
    ...
    private Call readOperator(Expression left, String op) {
        Call call = new Call(left.getLocation(), new Variable(left.getLocation(), "local." + op));
    ...
    private Call readUnaryOp(Expression ex, String op) {
        return new Call(ex.getLocation(), new Variable(ex.getLocation(), "local." + op), Collections.singletonList(ex));
    }

    private Call readCall(Expression name) {
        ...
        return new Call(name.getLocation(), name, args);
    }
    ...
    private Import readImport() {
        ...
        nextValue(WordToken.class, "from");
        ...
        return new Import(name.getLocation(), name.getValue(), module.getValue());
    }
